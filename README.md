Backend Engineer (Python/Django/DRF). Focused on building scalable REST APIs and modular backend systems. Interested in clean architecture, system design, and production-ready solutions.
## Tech Stack

- **Backend:** Python, Django, Django REST Framework (DRF), Flask
- **Databases:** PostgreSQL, MSSQL
- **Infrastructure & DevOps:** Docker, Docker Compose, Docker SWARM, Docker Networking, Redis, Celery, Nginx, GitHub Actions, CI/CD
- **Architecture & Design:** REST API, Domain-Driven Design (DDD), Modular Architecture
- **Tools:** Git, Linux, Postman, curl
- **Testing:** Pytest (unit/integration/e2e tests)
- **Other:** OOP, HTTP, SQL, NoSQL, Async Tasks, API Integration
